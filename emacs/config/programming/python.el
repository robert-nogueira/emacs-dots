...
(use-package python
  ...
  :hook
  (python-mode . (lambda ()
                   (if (poetry-venv-exist-p)
                       (progn
                         (message "Using Poetry virtual environment")
...
(add-hook 'python-mode-hook (lambda () (setq-local company-backends '(company-capf company-dabbrev))))
